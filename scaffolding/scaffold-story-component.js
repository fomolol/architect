...
import dopl from 'dopl'

export default ({ name, path, src, dest }) => {
...
const createClassName = (name) => {
  ...
  return name.replace(/[A-Z]/g, (word) => '_' + word.toLowerCase()).substring(1) // trim off leading - from the first capital
}
